...
(function ($) {
	...
	$(document).on('click', '.wbtm_settings_pricing_routing .wbtm_stop_item .mp_item_remove', function (e) {
		if (e.result) {
			wbtm_reload_pricing($('.wbtm_settings_pricing_routing'));
		}
	});
	$(document).on('change', '.wbtm_settings_pricing_routing [name="wbtm_route_place[]"],.wbtm_settings_pricing_routing [name="wbtm_route_type[]"]', function () {
		wbtm_reload_pricing($('.wbtm_settings_pricing_routing'));
	});
}(jQuery));
//==========Seat plan settings=================//
(function ($) {
	"use strict";
	$(document).on('click', '.wbtm_settings_seat .wbtm_create_seat_plan', function (e) {
	...
	$(document).on('click', '.wbtm_settings_seat .wbtm_seat_plan_preview .mp_item_remove', function (e) {
		if (e.result) {
	...
	$(document).on('click', '.wbtm_settings_seat .wbtm_seat_plan_preview .mp_add_item', function (e) {
		if (e.result) {
			let parent=$('.wbtm_settings_seat');
			let target = parent.find('[name="wbtm_seat_rows"]');
			let value = parseInt(target.val())+1;
	...
	//=============================//
	$(document).on('click', '.wbtm_settings_seat .wbtm_create_seat_plan_dd', function (e) {
		let parent = $(this).closest('.wbtm_settings_seat');
	...
	$(document).on('click', '.wbtm_settings_seat .wbtm_seat_plan_preview_dd .mp_item_remove', function (e) {
		if (e.result) {
			let parent=$('.wbtm_settings_seat');
	...
	$(document).on('click', '.wbtm_settings_seat .wbtm_seat_plan_preview_dd .mp_add_item', function (e) {
		if (e.result) {
			let parent=$('.wbtm_settings_seat');
...
(function ($) {
	...
	$(document).on('click', '.wbtm_add_group_pickup', function () {
		...
		target_item.find('[name*="wbtm_bp_pickup"]').attr('name', 'wbtm_bp_pickup[' + unique_id + ']');
