	...
	if ( ! class_exists( 'WBTM_Custom_Slider' ) ) {
		class WBTM_Custom_Slider {
			...
			public function slider( $post_id, $image_ids ) {
				if ( is_array( $image_ids ) && sizeof( $image_ids ) > 0 ) {
					$showcase_position = WBTM_Global_Function::get_slider_settings( 'showcase_position', 'right' );
					$column_class      = $showcase_position == 'top' || $showcase_position == 'bottom' ? 'area_column' : '';
					$slider_style      = WBTM_Global_Function::get_slider_settings( 'slider_style', 'style_1' );
					?>
                    <div class="superSlider placeholder_area fdColumn">
						...
						<?php
							$slider_indicator = WBTM_Global_Function::get_slider_settings( 'indicator_visible', 'on' );
							$icon             = WBTM_Global_Function::get_slider_settings( 'indicator_type', 'icon' );
			...
			public function post_thumbnail( $image_id = '' ) {
				$thumbnail = WBTM_Global_Function::get_image_url( '', $image_id );
				if ( $thumbnail ) {
					?>
                    ...
                    <div class="sliderAllItem">
						<?php
							...
							foreach ( $image_ids as $id ) {
								$image_url = WBTM_Global_Function::get_image_url( '', $id );
								?>
			...
			public function slider_showcase( $image_ids ) {
				$showcase = WBTM_Global_Function::get_slider_settings( 'showcase_visible', 'on' );
				...
				if ( is_array( $image_ids ) && sizeof( $image_ids ) > 0 ) {
                    ...
                    <div class="slideIndicator">
						<?php
							$count = 1;
							foreach ( $image_ids as $id ) {
								$image_url = WBTM_Global_Function::get_image_url( '', $id, array( 150, 100 ) );
								?>
			...
			public function slider_popup( $post_id, $image_ids ) {
				if ( is_array( $image_ids ) && sizeof( $image_ids ) > 0 ) {
					$popup_icon_indicator = WBTM_Global_Function::get_slider_settings( 'popup_icon_indicator', 'on' );
					?>
                    <div class="sliderPopup" data-popup="superSlider">
                        <div class="superSlider">
                            ...
                            <div class="popupFooter">
								<?php
									$indicator = WBTM_Global_Function::get_slider_settings( 'popup_image_indicator', 'on' );
									if ( $indicator == 'on' ) {
										$this->image_indicator( $image_ids );
									}
								?>
			...
			public function get_slider_ids( $post_id, $key ) {
				...
				$image_ids = WBTM_Global_Function::get_post_info( $post_id, $key, array() );
				if ( $thumb_id ) {
