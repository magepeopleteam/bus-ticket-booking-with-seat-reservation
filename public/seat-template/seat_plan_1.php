    ...
    if ($wbtmmain->wbtm_buffer_time_check($bp_time, $date) == 'yes') {
        ...
        <div class="bus-seat-panel">
            ...
            if ($numeric_seat) {
                ...
                ?>
                <table class="bus-seats" width="300" border="1" style="width: 211px; border: 0px solid #ddd;">
                    <?php foreach ($seat_row_arr as $seat_row) { ?>
                        <tr class="seat<?php echo get_the_id() . $wbtmmain->wbtm_make_id($date); ?>_lists ">
                            <?php foreach ($seat_col_arr as $seat_col) {
                                ...
                                $get_seat_status = $wbtmmain->wbtm_get_seat_status($seat_name, $date, get_the_id(), $b_start, $end);
                                if ($get_seat_status) {
                                ...
                                <td>
                                    ...
                                    <?php } else { ?>
                                        ...
                                        <a data-seat='<?php echo $seat_row . $seat_col; ?>' id='seat<?php echo get_the_id() . $wbtmmain->wbtm_make_id($date); ?>_<?php echo $seat_row . $seat_col; ?>' data-sclass='Economic' class='seat<?php echo get_the_id() . $wbtmmain->wbtm_make_id($date); ?>_blank blank_seat'><?php echo $seat_row . $seat_col; ?></a>
                                    <?php } ?>
                                </td>
                                <?php if ($seat_col == $seat_style) {
                                    echo "<td class='no-border empty-lane'></td>";
                                } ?>
                                <?php $count++;
                            } ?>
                        </tr>
