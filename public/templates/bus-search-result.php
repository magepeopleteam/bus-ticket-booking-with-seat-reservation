...
<tr class="<?php echo $wbtmmain->wbtm_find_product_in_cart(get_the_id()); ?>">
            ...
            <td ><?php echo get_woocommerce_currency_symbol(); ?><?php echo $wbtmmain->wbtm_get_bus_price($start,$end, $price_arr); ?></td>
            ...
            <td><button id="view_panel_<?php echo get_the_id().$wbtmmain->wbtm_make_id($date); ?>" class='view-seat-btn'>
            <?php echo $wbtmmain->bus_get_option('wbtm_view_seats_text', 'label_setting_sec') ? $wbtmmain->bus_get_option('wbtm_view_seats_text', 'label_setting_sec') : _e('View Seats','bus-ticket-booking-with-seat-reservation'); ?>    
            </button></td>
        </tr>
        <tr style='display: none;' class="admin-bus-details" id="admin-bus-details<?php echo get_the_id().$wbtmmain->wbtm_make_id($date); ?>">
            <td colspan="11">
            <?php
            $bus_meta           = get_post_custom(get_the_id());
            $seat_col           = $bus_meta['wbtm_seat_col'][0];
            $seat_row           = $bus_meta['wbtm_seat_row'][0];
            $next_stops_arr     =  maybe_unserialize(get_post_meta(get_the_id(), 'wbtm_bus_next_stops', true));
            $wbtm_bus_bp_stops  =  maybe_unserialize(get_post_meta(get_the_id(), 'wbtm_bus_bp_stops', true));
    ...
    <div>
        <div class="bus-seat-panel">
            <?php         
                ...
                $wbtmmain->wbtm_bus_seat_plan_dd($start,$date); 
            ?> 
        </div>
            ...
            <form action="" method='post'>
                <div class="top-search-section">                    
                    <div class="leaving-list">
                        ...
                        <h6>
                        <?php echo $wbtmmain->bus_get_option('wbtm_route_text', 'label_setting_sec') ? $wbtmmain->bus_get_option('wbtm_route_text', 'label_setting_sec') : _e('Route','bus-ticket-booking-with-seat-reservation'); ?>    
                        </h6>
                    ...
                    <div class="leaving-list">
                        <h6><?php echo $wbtmmain->bus_get_option('wbtm_date_text', 'label_setting_sec') ? $wbtmmain->bus_get_option('wbtm_date_text', 'label_setting_sec') : _e('Date','bus-ticket-booking-with-seat-reservation'); ?>
                        </h6>
                        <div class="selected_date">
                            <?php printf( '<span>%s</span>', date_i18n($date_format, strtotime( $date ) ) ); ?>
                        </div>
                    </div>   
                    <div class="leaving-list">
                        <h6>
                          <?php echo $wbtmmain->bus_get_option('wbtm_start_arrival_time_text', 'label_setting_sec') ? $wbtmmain->bus_get_option('wbtm_start_arrival_time_text', 'label_setting_sec') : _e('Start & Arrival Time','bus-ticket-booking-with-seat-reservation'); ?>  
                        </h6>
                        <div class="selected_date">
                            <?php                                                               
                ...
                <div class="seat-selected-list-fare">
                    <table class="selected-seat-list<?php echo get_the_id().$wbtmmain->wbtm_make_id($date); ?>">
                        <tr class='list_head<?php echo get_the_id().$wbtmmain->wbtm_make_id($date); ?>'>
                            <th>
                            <?php echo $wbtmmain->bus_get_option('wbtm_seat_no_text', 'label_setting_sec') ? $wbtmmain->bus_get_option('wbtm_seat_no_text', 'label_setting_sec') : _e('Seat No','bus-ticket-booking-with-seat-reservation'); 
                             ?>    
                            </th>
                            <th>
                             <?php echo $wbtmmain->bus_get_option('wbtm_fare_text', 'label_setting_sec') ? $wbtmmain->bus_get_option('wbtm_fare_text', 'label_setting_sec') : _e('Fare','bus-ticket-booking-with-seat-reservation'); 
                             ?>       
                            </th>
                            <th width='50'>
                              <?php echo $wbtmmain->bus_get_option('wbtm_remove_text', 'label_setting_sec') ? $wbtmmain->bus_get_option('wbtm_remove_text', 'label_setting_sec') : _e('Remove','bus-ticket-booking-with-seat-reservation'); 
                             ?>      
                            </th>
                        </tr>
                        <tr>
                            <td align="center">
                             ...
                             ?>     
                             <span id='total_seat<?php echo get_the_id().$wbtmmain->wbtm_make_id($date); ?>_booked'></span><input type="hidden" value="" id="tq<?php echo get_the_id().$wbtmmain->wbtm_make_id($date); ?>" name='total_seat' class="number"/></td>
                    ...
                    <button id='bus-booking-btn<?php echo get_the_id().$wbtmmain->wbtm_make_id($date); ?>' type="submit" name="add-to-cart" value="<?php echo esc_attr(get_the_id()); ?>" class="single_add_to_cart_button button alt btn-mep-event-cart">
