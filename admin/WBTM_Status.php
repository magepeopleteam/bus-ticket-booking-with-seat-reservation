	...
	if (!class_exists('WBTM_Status')) {
		class WBTM_Status {
			...
			public function status_menu() {
				...
				add_submenu_page('edit.php?post_type=' . $cpt, __('Status', 'bus-ticket-booking-with-seat-reservation'), __('Status', 'bus-ticket-booking-with-seat-reservation'), 'manage_options', 'wbtm_status_page', array($this, 'status_page'));
			}
			public function status_page() {
				$label = WBTM_Functions::get_name();
				$wc_i = WBTM_Global_Function::check_woocommerce();
				$wc_i_text = $wc_i == 1 ? esc_html__('Yes', 'bus-ticket-booking-with-seat-reservation') : esc_html__('No', 'bus-ticket-booking-with-seat-reservation');
				$wp_v = get_bloginfo('version');
				$wc_v = ( $wc_i == 1 && function_exists('WC') ) ? WC()->version : '';
				...
				<div class="wbtm_style">
					<?php do_action('wbtm_status_notice_sec'); ?>
					<div class=_dShadow_6_adminLayout">
						<h2 class="textCenter"><?php echo esc_html($label) . '  ' . esc_html__('For Woocommerce Environment Status', 'bus-ticket-booking-with-seat-reservation'); ?></h2>
						<div class="divider"></div>
						<table>
							<tbody>
							<tr>
								<th data-export-label="WC Version"><?php esc_html_e('WordPress Version : ', 'bus-ticket-booking-with-seat-reservation'); ?></th>
							...
							<tr>
								<th data-export-label="WC Version"><?php esc_html_e('Woocommerce Installed : ', 'bus-ticket-booking-with-seat-reservation'); ?></th>
							...
							<?php if ($wc_i == 1) { ?>
								<tr>
									<th data-export-label="WC Version"><?php esc_html_e('Woocommerce Version : ', 'bus-ticket-booking-with-seat-reservation'); ?></th>
								...
								<tr>
									<th data-export-label="WC Version"><?php esc_html_e('Name : ', 'bus-ticket-booking-with-seat-reservation'); ?></th>
								...
								<tr>
									<th data-export-label="WC Version"><?php esc_html_e('Email Address : ', 'bus-ticket-booking-with-seat-reservation'); ?></th>
