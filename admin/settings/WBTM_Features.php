...
if (!class_exists('WBTM_Features')) {
    ...
    $terms = get_terms([
        ...
        'hide_empty' => false, // Show all features, even if they have no posts
    ]);

    // Retrieve previously selected feature images
...
public function settings_save($post_id) {
    if (get_post_type($post_id) == WBTM_Functions::get_cpt()) {
        // Check if the selected_bus_features field is set
        if (isset($_POST['selected_bus_features'])) {
            // Sanitize and save the selected features to post meta
            ...
            // If no features were selected, delete the meta field
